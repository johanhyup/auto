...
def _cfg_path(key: str, default: str) -> Path:
    """
    config.toml [paths] 섹션에 정의된 상대경로를
    root_dir 기준 절대 Path 로 반환
    """
    return root_dir() / config.paths.get(key, default)


# storage / resource
def storage_dir(sub: str = "") -> str:
    p = _cfg_path("storage_tasks_dir", "storage") if not sub else _cfg_path("storage_tasks_dir", "storage") / sub
    p.mkdir(parents=True, exist_ok=True)
    return str(p)


# i18n 기능 제거 – 관련 함수/호출 모두 삭제하거나 주석 처리
# def resource_dir(sub: str = "") -> str:
#     p = _cfg_path("resource_dir", "resource")
#     if sub:
#         p = p / sub
#     p.mkdir(parents=True, exist_ok=True)
#     return str(p)
...
def task_dir(sub: str = "") -> str:
    p = _cfg_path("storage_tasks_dir", "storage/tasks")
    if sub:
        p = p / sub
    p.mkdir(parents=True, exist_ok=True)
    return str(p)
...
def font_dir(sub: str = "") -> str:
    p = _cfg_path("fonts_dir", "resource/fonts")
    if sub:
        p = p / sub
    p.mkdir(parents=True, exist_ok=True)
    return str(p)
...
def song_dir(sub: str = "") -> str:
    p = _cfg_path("songs_dir", "resource/songs")
    if sub:
        p = p / sub
    p.mkdir(parents=True, exist_ok=True)
    return str(p)
...
def public_dir(sub: str = "") -> str:
    p = _cfg_path("public_dir", "resource/public")
    if sub:
        p = p / sub
    p.mkdir(parents=True, exist_ok=True)
    return str(p)
...
# utils.py (Modified: Added srt_time_to_seconds function for parsing subtitle times)
# Note: Assuming utils.py exists, add this function if not present. Since not provided, include as part of response.
